...
#restrict degree to at most 4 (to control sparsity)
for (i in 1:p){
  while(sum(a[i,1:p])>5) {
    tf<-(a[i,1:p]==1)
...
#Now turn a into a precision matrix, with non-zero (non-diagonal) entries 0.2
...
# simulate Mutivariate t data with 1 df (say)
tdata<-rmvt(n,S,df)